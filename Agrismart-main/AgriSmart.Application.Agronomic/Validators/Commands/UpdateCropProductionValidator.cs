...
{
    public class UpdateCropProductionValidator : BaseValidator<UpdateCropProductionCommand>
    {
        public UpdateCropProductionValidator()
        {
            RuleFor(x => x).Must(AreFiltersValid).WithMessage(x => x.GetType().Name.ToString() + " parameters are invalid");
        }

        protected override bool AreFiltersValid(UpdateCropProductionCommand command)
        {
