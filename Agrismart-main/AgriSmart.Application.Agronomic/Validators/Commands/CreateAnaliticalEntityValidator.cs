...
{
    public class CreateAnaliticalEntityValidator : BaseValidator<CreateAnaliticalEntityCommand>
    {
        public CreateAnaliticalEntityValidator()
        {
            RuleFor(x => x).Must(AreFiltersValid).WithMessage(x => x.GetType().Name.ToString() + " parameters are invalid");
        }

        protected override bool AreFiltersValid(CreateAnaliticalEntityCommand command)
        {
