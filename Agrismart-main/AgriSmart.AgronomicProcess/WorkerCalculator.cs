...
namespace AgriSmart.AgronomicProcess
{
    public class WorkerCalculator : BackgroundService
    {
        ...
        public WorkerCalculator(ILogger<WorkerRawData> logger, IOptions<AgronomicProcessConfiguration> agronomicProcessConfiguration, IOptions<AgrismartApiConfiguration> agrismartApiConfiguration, IAgriSmartApiClient agriSmartApiClient)
        {
            _logger = logger;
        ...
        {
            _logger.LogInformation("Worker Calculator started at: {time}", DateTimeOffset.Now);
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (_session.RoleId == 0)
                    {
                        ...
                        CalculationsProcess calculationsProcess = new CalculationsProcess(_logger, _agrismartApiConfiguration, _agriSmartApiClient);
                        await calculationsProcess.Calculate(await businessEntity.GetCropProductionEntities(_session.Token), _session.Token, stoppingToken);
                    }
                }
                catch (Exception ex)
                {
